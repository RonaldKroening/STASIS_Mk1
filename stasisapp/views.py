...
api_key = str(os.environ.get('avap'))

# https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=IBM&outputsize=full&apikey=demo

def get_data(request):
    tckr = request.GET.get('tckr', 'GOOG')
    data = get_info(tckr)
    print("request from stasisapp/views.py: ",request)
    print(request.GET.get('tckr'))
    response_data = {
        'ticker_data': data,
    }
...
def get_client():
    ...
    uri = "mongodb+srv://<YOURUSER>:"+password+"@cluster0.0qo2iwi.mongodb.net/?retryWrites=true&w=majority&appName=AtlasApp"
    ...
    for item in MD.find():
        ...
        if item['TCKR'] == ticker:
            ...
            if markIdx == None: # If 'marketItems' doesn't exist, fetch the financial data from Polygon.io
                r = requests.get(f"https://api.polygon.io/vX/reference/financials?ticker="+ticker+"&apiKey="+apikey)
                response = r.json()
